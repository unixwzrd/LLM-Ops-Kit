# Documentation Index

Back: [docs/README.md](./README.md)

- [Documentation Index](#documentation-index)
  - [Start Here](#start-here)
  - [Models and Runtime](#models-and-runtime)
  - [TTS and Audio](#tts-and-audio)
  - [Deployment and Ops](#deployment-and-ops)
  - [Reference](#reference)


This is the canonical index for operator docs in `LLM-Ops-Kit`.

## Start Here

- [Quickstart](./QUICKSTART.md)
- [How It Works](./HOW_IT_WORKS.md)
- [Switching Models and Agents](./SWITCHING.md)
- [Configuration](./CONFIGURATION.md)
- [Troubleshooting](./TROUBLESHOOTING.md)
- [Architecture](./ARCHITECTURE.md)

## Models and Runtime

- [Adding a Model Profile](./ADDING_MODEL_PROFILE.md)
- [modelctl Guide](./MODELCTL_GUIDE.md)
- [Proxy Tap Runbook](./PROXY_TAP_RUNBOOK.md)

## TTS and Audio

- [MLX Audio TTS Guide](./MLX_AUDIO_TTS_GUIDE.md)
- [scripts/tts.md](./scripts/tts.md)
- [scripts/tts-bridge.md](./scripts/tts-bridge.md)

## Deployment and Ops

- [Deployment Sync Runbook](./DEPLOYMENT_SYNC_RUNBOOK.md)
- [SSH Setup Runbook](./SSH_SETUP_RUNBOOK.md)
- [Release Audit Checklist](./RELEASE_AUDIT_CHECKLIST.md)
- [scripts/README.md](./scripts/README.md)

## Reference

- [Glossary](./GLOSSARY.md)
