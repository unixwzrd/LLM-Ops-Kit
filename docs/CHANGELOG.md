...
- **Scope:** `agent-work/scripts`, `agent-work/docs`
...
- **What changed:**
  ...
  - Updated `scripts/sync-agent-work.sh` to auto-refresh the manifest before rsync.
  - Added `--no-manifest` flag to `sync-agent-work.sh` to skip auto-generation when needed.
...
- **Behavior notes:**
  - Runtime link entries are now auto-generated from launcher symlinks via `scripts/generate-manifest` (run automatically by `sync-agent-work`).
