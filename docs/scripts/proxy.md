...
- Wrapper persists live runtime metadata under `~/.llmops/run/proxy-live-*` for human-readable `status`.
...
~/bin/proxy restart --upstream <upstream-host>:<upstream-port> --raw-log ~/.llmops/logs/openai-proxy.raw.log
~/bin/proxy restart --upstream <upstream-host>:<upstream-port> --raw-request-log ~/.llmops/logs/openai-proxy.requests.log --raw-response-log ~/.llmops/logs/openai-proxy.responses.log
```

## Notes

- Use `status` to check process state (`running/stopped/stale`).
- `status` reads persisted live settings from `~/.llmops/run/proxy-live-*` and reports health via `/v1/models`.
...
- Default framed raw logging is combined into `~/.llmops/logs/openai-proxy.raw.log` unless explicitly split.
