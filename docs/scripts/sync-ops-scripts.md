...
Sync LLM-Ops-Kit to a remote host over SSH, refresh runtime link manifest, then
deploy and verify runtime links on the remote host in one run.
...
- Host defaults are loaded from `scripts/config/hosts.env`, optional `scripts/config/hosts.local.env`, and `~/.config/llm-ops/config.env` via shared shell env loading.
