...
- Wrapper persists live runtime metadata under `~/.local/state/llm-ops/run/model-proxy-live-*` for readable `status` output.
...
- `status` reads persisted live settings from `~/.local/state/llm-ops/run/model-proxy-live-*`, reports the effective `listen` URL and `upstream`, shows the live listener PID when available, and probes health via `/v1/models`.
- `model-proxy` does not use a dedicated standalone config file; it is driven by env and CLI flags.
- `--upstream` is required unless `LLMOPS_UPSTREAM_HOST` and `LLMOPS_UPSTREAM_PORT` are set in `~/.config/llm-ops/config.env`.
...
- Default framed raw logging is combined into `~/.local/state/llm-ops/logs/model-proxy.raw.log` unless explicitly split.
